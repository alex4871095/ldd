...
static int value_set(const char *val, const struct kernel_param *kp)
{
	...
	printk(KERN_ALERT "new value for %s\n", kp->name);

	return 0;
}

static int value_get(char *buf, const struct kernel_param *kp)
{

        param_get_int(buf, kp);
...
module_param_cb(value1, &pops, &value1, 0664);
